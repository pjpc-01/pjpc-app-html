...
> 最后更新：2026-08-08（薪资SOCSO/EIS修正 + Synorex导入 + 成绩部分导入 + NFC待统一）
...
| 3 | **Finance 全测试** | 薪资 SOCSO/EIS 刚修正，要出 payslip 了必须对 | SOCSO 换成 PERKESO 2024 表 ✅、EIS 去 2.45 上限 ✅、Synorex 97条 ✅、PCB 累进税率待验证 |
...
- **布局组件：** `components/layouts/AppShell.tsx`（统一侧边栏布局，无独立顶栏）
...
- **✅ 已修复：双 header 问题** — AppShell 不再渲染顶部 header，PageLayout 去掉角色/状态 badge，`/admin` 旧 layout 改为透传

---

## 三、现在做到哪了？（2026-06-20 快照）

### 📊 整体进度（2026-07-13 审计更新）
...
| 家长端 | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 100% | 独立管理+家长门户 6 页（含dailylogs） |
| 库存模块 | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 100% | 进销存管理 |
| 课程打磨 | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 100% | 真实PB数据+冲突检测+班级管理 |
| 🔴 安亲班核心 | 🟩🟩🟩🟩🟩🟩🟩🟩⬜⬜ 80% | 每日日志✅ 成绩UI✅(215行) 接送✅(233行) 随手拍❌ |
| 🔵 NFC/考勤 | 🟩🟩🟩🟩🟩🟩🟩🟩🟩⬜ 90% | 双轨制✅ 统一考勤✅ 积分联动✅ 积分active过滤❌ |
| 🎨 体验升级 | 🟩🟩🟩🟩🟩⬜⬜⬜⬜⬜ 50% | Toast✅ 双header✅ CmdK❌ 暗色❌ 骨架屏❌ 空状态❌ |
| 🧹 代码清理 | 🟩🟩🟩🟩🟩🟩🟩⬜⬜⬜ 70% | 死代码-13K行✅ 但仍有1034行student-mgmt+11个app/components |
| 企业级 | ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜ 5% | 审计日志/备份/通知中心 |
...
| `/admin` | ✅ | 系统仪表板（无旧 layout 冲突） |
| `/admin/students` | ❌ 已删 | 旧版独立 layout 页面，由 `/student-management` 替代 |
...
考勤:    /api/attendance /api/attendance/logs /api/attendance/report /api/attendance/checkin /api/attendance/settings /api/attendance/teacher /api/attendance/student /api/attendance/person-calendar /api/attendance/monthly /api/attendance/analytics
```

### 🔍 2026-07-13 审计 — 蓝图 vs 实际代码差距

| 蓝图声称 | 实际状态 | 差距 |
|---------|---------|------|
| grades "PB已建无UI" | ✅ **215行完整UI**（`/grades`，含useGrades hook、统计、编辑） | 蓝图过时 |
| 接送管理 "⏳ 未做" | ✅ **233行完整UI**（`/pickup`，含usePickup hook、接送人管理） | 蓝图过时 |
| 家长端 "5 页" | ✅ **6 页**（含 `/parent/dailylogs` 182行） | 蓝图过时 |
| 体验升级 90% | ⚠️ 实际 ~50%：暗色❌ CmdK❌ 骨架屏❌ 空状态❌ | 蓝图高估 |
| 代码清理 90% | ⚠️ 实际 ~70%：1034行旧student-mgmt仍在 `app/components/`，11个目录未迁移 | 蓝图高估 |
| NFC/考勤 80% | ✅ 实际 ~90%：4 bugs今日修复 + logs API已建 | 蓝图低估 |
| 安亲班核心 40% | ⚠️ 实际 ~80%：每日日志✅ 成绩✅ 接送✅ 仅随手拍缺失 | 蓝图严重低估 |
| student.center→centerId | ❌ 10 个文件仍引用旧 `student.center` 字段 | 数据一致性 |
| 积分 active 过滤 | ❌ 未过滤 status=active（缺勤/毕业学生仍出现在积分页） | 功能Bug |
| 暗色模式 | ❌ 零代码 | 新需求 |
| 全局搜索 Cmd+K | ❌ cmdk 在 node_modules 但无自定义组件 | 新需求 |
| 资源库 | ⚠️ 仅 40 行骨架，无实质功能 | 占位符 |
...
| 数据表格统一（筛选/排序/搜索/分页） | ⏳ | |
| **删除旧版 admin/layout.tsx 独立 sidebar+header** | ✅ 2026-06-23 | admin/layout.tsx 改为透传，消除双 layout 冲突 |
...
| **不一致的布局** | ✅ 已修复 | 统一 AppShell 为唯一布局，PageLayout 仅作内容组件 |
| **双 Header 问题** | ✅ 已修复 | AppShell 去掉顶部 header，PageLayout 去掉角色/状态 badge，admin layout 改为透传 |
...
| **薪资管理** | ✅ payroll | ✅ SOCSO→PERKESO 2024 bracket表, EIS去2.45上限, Synorex导入97条 |
...
| # | 功能 | 为什么 |
|---|------|--------|
| 1 | **PageLayout → AppShell 统一** | ✅ 已修复：AppShell 去掉顶栏，admin layout 透传，PageLayout 去掉标签 |
| 2 | **暗色模式** | 一行 CSS + next-themes |
| 3 | **全局搜索 Cmd+K** | 找学生/教师/发票，一个框解决 |
| 4 | **空状态 + 骨架屏** | 列表无数据时显示引导 |
| 5 | **品牌色更新** | ✅ 2026-06-23 | indigo/amber → 黄+褐+奶油（取自装修照片） |

---

### 🔵 Phase 5f：安亲班专属（我决定，第六优先 🚧 进行中 2026-06-23）

| # | 功能 | 价值 | 状态 |\n|---|------|------|:----:|\n| 1 | 📓 **每日日志** `daily_logs` | 安亲班核心产品 | ✅ 教师端323行 + 家长端182行 |\n| 2 | 📊 **成绩管理** `grades` | 对接现有 homework 系统 | ✅ 215行UI + useGrades hook |\n| 3 | 🚗 **接送管理** | 刚需 | ✅ 233行UI + usePickup hook |\n| 4 | 📸 **随手拍推家长** | 家长粘性 | ❌ 未做 |\n\n**5f-1 每日日志 ✅：**\n- PB `daily_logs` collection + API + Hook + 教师端 `/daily-logs` (323行) + 家长端 `/parent/dailylogs` (182行)\n\n**5f-2 成绩管理 ✅（2026-07-13 审计发现已完成）：**\n- PB `grades` collection + `/app/grades/page.tsx` (215行) + `useGrades` hook\n- 功能：科目/学期筛选 + 录入/编辑成绩 + 统计（最高/最低/平均/及格率）+ 年级字母颜色\n\n**5f-3 接送管理 ✅（2026-07-13 审计发现已完成）：**\n- `/app/pickup/page.tsx` (233行) + `usePickup` hook\n- 功能：接送人管理 + 关系类型 + 接送记录 + 今日预计接送列表

---

### 🟢 Phase 5g：NFC 双轨制 + 考勤大改（✅ 2026-07-13 更新）
...
| 7 | 🆕 **顶部统计恒为0** | `api/attendance/logs/route.ts` | 新建 logs API，修复打卡人数/签到/签退始终显示0 |
| 8 | 🆕 **NaNm 时长显示** | `UnifiedAttendanceHub.tsx:290` | `check_in_time`→`check_in`，修复 `new Date("13:39")` → NaN |
| 9 | 🆕 **教师报告0数据** | `teacher-attendance-reports/page.tsx:153` | `data.teachers`→`data.data`，修复 API 返回字段不匹配 |
| 10 | 🆕 **教师考勤无记录** | 连锁修复 | Bug 3 导致 teachers.length=0 → useEffect 不触发 |
...
### 品牌色（2026-06-23 更新 — 取自装修照片）

| 颜色 | 值 | 用在哪 |
|------|-----|--------|
| `yellow-400` | `#FACC15` | 主色 — Logo/按钮/侧边栏激活 |
| `yellow-50` | `#FEFCE8` | 背景 — 侧边栏选中/hover |
| `yellow-100` | `#FEF9C3` | 背景 — 分行Tab active |
| brown `#5c4a30` | `#5C4A30` | 文字 — 侧边栏正文/标题 |
| brown `#7a6a48` | `#7A6A48` | 文字 — 侧边栏次要/图标 |
| 奶油 `#f5f0e5` | `#F5F0E5` | 侧边栏背景 |
...
|---------|------------|-----------|:--:|
| **配色方案** | 单一主色+灰度系统 (slate/neutral) | ✅ 黄+褐+奶油品牌色系统（取自装修照片） | 接近 ✅ |
...
2. ✅ admin/layout.tsx 改为透传（不再渲染旧 sidebar/header），旧 404 链接自动失效
...
#### 批次 4：UI 美化（部分完成 ✅ 2026-06-23）

```
1. ✅ 品牌色从 indigo/amber 改为 黄+褐+奶油（取自装修照片）
2. ✅ 双 header 消灭：AppShell 去掉顶栏，admin layout 透传
3. ✅ PageLayout 去掉角色/状态 badge
4. ⏳ 全局统一 rounded-xl + shadow-sm + border-slate-100
5. ⏳ 图表 6 色色板
6. ⏳ 表格操作按钮 hover 可见
7. ⏳ 侧边栏激活态圆点指示器（已有部分）
```
...
*审计完成时间：2026-06-23 | 审计者：deepseek-v4-pro | 下一步：执行批次 3 安亲班核心功能 | UI 批次 4 部分完成（品牌色+双header）*
...
## 九、2026-07-13 审计后 — 下一步优先级

> 按用户真实需求 + 投入产出比排序。每次做一个，做完更新蓝图。

| # | 任务 | 难度 | 影响 | 说明 |
|---|------|:---:|:---:|------|
| 1 | 🌙 **暗色模式** | ⭐ | 高 | next-themes + 几行CSS，教师晚上用护眼，立竿见影 |
| 2 | 🎯 **积分 status=active 过滤** | ⭐ | 中 | `/points` 页排除毕业/停学学生，数据准确性 |
| 3 | 🔍 **Cmd+K 全局搜索** | ⭐⭐ | 高 | cmdk 已在 node_modules，只需写 CommandPalette 组件 |
| 4 | 🧹 **删除死代码** | ⭐ | 中 | 1034行 `app/components/management/student-management-page.tsx` 已被 components/students/ 替代 |
| 5 | ⬜ **骨架屏 + 空状态** | ⭐⭐ | 中 | 列表加载时 Skeleton，空数据时 EmptyState |
| 6 | 🏗️ **app/components/ → components/ 迁移** | ⭐⭐ | 低 | 11个目录从错误位置移到正确位置 |
| 7 | 📸 **随手拍推家长** | ⭐⭐⭐ | 高 | 老师拍照→选学生→一键推送家长端通知 |
| 8 | 🔗 **student.center → centerId** | ⭐⭐ | 中 | 10个文件统一字段引用 |
| 9 | 📦 **资源库完善** | ⭐⭐⭐ | 中 | 40行骨架→完整文件管理 |
| 10 | 🔐 **企业级** | ⭐⭐⭐⭐ | 低 | 审计日志/自动备份/通知中心 |
