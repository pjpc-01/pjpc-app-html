...
> 每周一 cron「PJPC 每周 UX 检查」自动复验，行尾「最后验证」= 最近一次真浏览器实测日期。

- **两个空壳页**：`接送管理`（空）、`预算管理`（空）→ 要真做还是置灰？（最后验证：2026-09-25，仍然存在：两页只有空状态 + 一个入口按钮，无任何数据）
- **菜单 39 项全平铺、没有二级分组**（教务 15 项、财务 12 项、系统 9 项）（最后验证：2026-09-25，仍然存在）
...
- **单页按钮过多**（每行操作列重复堆）：家长管理 543 个、学生列表 282、学生费用 244、薪资管理 174、发票管理 130、支出管理 129、课程管理 119、付款和收据 116（最后验证：2026-09-25，仍然存在）
- 🆕 **教育概览「今日课节」恒为 0、课程表永远空白**（2026-09-25 新发现）
  - 根因：`app/education/page.tsx` 只 `useSchedule()` 拿数据，**从没调用 `fetchSchedules()`**（全项目唯一消费者）→ `schedules` 永远是 `[]`，所以「今日课节」永远 0、月历上永远没有课节
  - 证据：真浏览器打开该页 **零次 `/api/schedule` 请求**；而 `schedules` 表 2026-09-25 当天有 **10 条**记录
- 🆕 **教育概览「班级总览」全部年级 0 人、128 人全塞进「未分年级」**（2026-09-25 新发现）
  - 根因：`lib/pocketbase-students.ts` 的 `getAllStudents()` 格式化映射**漏了 `grade` 字段**（只映射了 `standard`）→ `hooks/useStudents` 归一后 `grade` 恒为 `''` → 该页用 `s.grade` 分组全部落空（年级分组列表因此全靠课程撑门面，每个年级「0 人」）
- 🆕 **教育概览统计「课程 50」≠ 实际 81 门**（2026-09-25 新发现）
  - 根因：`hooks/useCourses` → `/api/courses` 默认 `per_page=50` 且没传分页参数 → 只拿到前 50 门（DB 共 81 门），班级总览里的课程清单也跟着缺
- 🆕 **系统设置「活跃用户 0 / 待审核 20」**（2026-09-25 新发现）
  - 根因：`app/settings/page.tsx:215-216` 读 `u.status` / `u.approved`，而 **users 集合根本没这两个字段**（只有 `verified`）→ 永远显示「活跃 0 / 待审核 20」，与「用户与权限管理」页 20 个正常用户、管理中心 20 用户矛盾
- 🆕 **生产库残留测试数据**（2026-09-25 新发现，未改数据只记录）
  - `activities` 2 条：`test`（2026-08-29 · 全中心）、`__DIAG_TEST__`（2026-09-15 · PU1）→ 出现在「活动管理」日历（15 日格）和教育概览「最新公告」
  - `point_logs` 35 条 `reason='test'`（2026-07-31 批量每人 -4）→ 留在学生积分历史里
- **`/student-reports` 加载偏慢**（最后验证：2026-09-25，2.6 秒时仍在「加载中…」，6 秒内正常出 107 份报告；**慢，不是坏**）
...
### 🧪 Tester 全站巡检结果（2026-09-25，手动跑）
**`node scripts/ux-audit.mjs`（真 Chromium 走 38 页、只读、superuser 代登录）**
- **38 / 38 页面：0 控制台错误、0 失败请求、0 异常重定向** ✅
- 3 页文本偏短，均为正常：`/student-reports`（抓到时还在「加载中…」，**该页加载偏慢**）、`/pickup`（今日暂无记录）、`/finance/budget`（未设预算）
- 禁用按钮 119 个（设计上「死入口改灰不可点」，属预期；其中「学生费用」110 个是未进编辑态）
- 截图 `/tmp/ux-audit/shots/`，原始 JSON `/tmp/ux-audit/audit.json`

**`node scripts/account-audit.mjs`（只读财务审计）**
- ✅ **所有发票都有账期**（验证本轮「空账期杂单」修复生效）｜✅ 票号唯一｜✅ 收款都有对应发票｜✅ 支出都挂了分行｜✅ 凭证编号完整（收款 108/108、支出 111/111）
- ⚠️ **2 张票收付金额不一致（历史，非本轮）**：`INV-202608-095` 票额 418 / 实收 390（差 -28）；`INV-202608-003` 票额 418 / 实收 420（差 +2）
- ⚠️ **11 笔支出没有凭证/收据附件** → 报税无法举证（会计大忌，用户待补）
- ⚠️ 银行流水 0 条（账户建了 2 个）→ 银行对账页无真实数据；无现金账户记录（现金收款 6 笔无法核对）
- ⚠️ 账套缺口：资产负债表仅简版（缺银行余额/现金/应付/固定资产/股东权益）、无预收款概念、无固定资产、无应付账款、无税务报表（SST/所得税/PCB 汇总）
- ⚠️ 数据质量：4 个学生学号不完整；**14 个学生没有年级**；**38 条薪资没有分行**（→ 分行损益不准）

### ⚠️「PJPC 每周 UX 检查」cron 曾挂（2026-09-21 → 2026-09-25 已恢复）
...
- ✅ **2026-09-25 10:04 已恢复**：本轮 cron 一次跑通（38 页 / 0 错误），新发现见文末「🔁 每周 UX 巡检复跑（cron 自动跑，2026-09-25 10:04）」
...
### 🔁 每周 UX 巡检复跑（cron 自动跑，2026-09-25 10:04）
- **`node scripts/ux-audit.mjs`：38 页 · 0 控制台错误 · 0 失败请求 · 0 异常重定向**
- 与同日 09:54 那次逐页比对，**唯一变化**是 `/student-reports`（2.6 秒抓取时仍在「加载中…」；另跑 3 次实测 6 秒内正常出 107 份报告 → **慢，不是坏**）
- 本轮 **5 条新发现**已写进「## 五、UX」：
  1. 教育概览「今日课节」恒 0 / 课程表永远空（页面从没调 `fetchSchedules()`，实测该页零次 `/api/schedule` 请求）
  2. 教育概览「班级总览」每个年级 0 人、128 人全进「未分年级」（`getAllStudents()` 漏映射 `grade`）
  3. 教育概览「课程 50」≠ 实际 81 门（`/api/courses` 默认 `per_page=50` 截断）
  4. 系统设置「活跃用户 0 / 待审核 20」（读 users 集合不存在的 `status`/`approved` 字段）
...
### ⚠️ 更正：`Reset`/`resettt` 误删 → 已恢复（2026-09-25）
**我判断错了**：把 `reason='Reset'`(64) / `'resettt'`(80) 当成「测试垃圾」一并删除，**实际是老板手动批量清零积分的手动操作记录**。
- 已从备份**全量恢复 144 条**（`Reset` 64 → 2026-07-31；`resettt` 80 → 2026-08-02）
- PB 的 API 层 `created` 是只读（PATCH 被忽略）→ 直改 `pb_data_store/data.db` 复原时间戳（改前备份 `~/backups/pb-data.db.bak-20260925_114206`），API 读回一致
- **学生积分未受影响**（未动 `students.points`）
- ✅ 老板已确认（2026-09-25）**不用恢复**。仍保持删除的只有：`point_logs` 的 `test` 35 条 + `activities` 2 条（后者全表仅这 2 条，本就是测试建档；前者含 `amount=±1e53` 浮点损坏，且已有「系统补回」条目补偿过）
- **教训：删积分流水前必须先问。** 即使老板说「全做」，我给出的前提（"像垃圾"）是错的，就该在动手前更正前提再确认。
...
## ✅ 旧发票无损瘦身（2026-09-26）—— 215 张 1.90 GB → 56 MB（省 97.1%）

**做法**：不重新生成，只把每张 PDF 里那张**未压缩 RGB 位图流**用 `zlib`(Flate) 压缩 → **无损**、布局/尺寸/颜色/其他对象全不动 → 外观**像素级不变**。
**脚本**：`/home/pjpc/shrink-invoices.py`（支持多页；按对象 `idnum` 去重 —— 多页会共用同一图像对象，重复处理会被校验拦下）
**安全机制**：每张先写 `.shrinking` 临时文件 → **重新读回并逐字节比对像素** → 只有验证通过才覆盖原件；不过就丢弃并记错。
**结果**：**215/215** 可正常打开、页数与原版一致、0 问题；平均 **9.06 MB → 0.26 MB**；应用仍正常提供（`GET /invoices/... → 200`）。
**原件备份**：`/home/pjpc/backups/invoices-before-shrink-20260926_132523/`（2.0 GB）
**说明**：新生成的发票本来就小（本次 A 方案改成 JPEG），无需再瘦身。

## ✅ 每周巡检清单加入新页面（2026-09-26）
`scripts/ux-audit.mjs` 的 NAV 加入 `['财务/报销单', '/claim-form']` → 每周 cron 巡检从此覆盖 alicia 新做的报销单页（原来 38 页不含它）。


## ✅ 学生报告列表 —— 新增「填写进度」列（2026-09-26）

**需求**：报告的进度看板 —— 每份报告右侧显示「已填几个框 / 还有几个没填」。
**框的定义（11 个）**：成长寄语 · 科目成绩 · 老师评语 · 功课班评语 · 存在问题 · 改进建议 · 学业目标 · 能力目标 · 品格目标 · 学期总结 · 活动参与（= `student_reports` 里老师在编辑面板填的 11 个字段，**不含** `language` 设置项）
**实现**：`app/student-reports/page.tsx` 加 `REPORT_FIELDS` / `isFilled()` / `reportProgress()`，表格「操作」列前新增「填写进度」列。
- 全填完 → **绿色**；还有空 → **琥珀色**；鼠标悬停显示**具体缺哪几项** ✓
- **纯前端计算，不改数据结构** ✓（列表接口本来就取全字段）
- ⚠️ **科目成绩的特殊规则**：模板会预填科目名，只有「至少一个科目填了期中/期末分数」才算已填（否则报表会误判为已填）

**实测（真 Chromium，107 份报告）**：表头 `照片|学生|年级|学期|年份|状态|创建时间|填写进度|操作` ✓ 零控制台错误 ✓
**分布**：已填 10/11 → 26 份 · 9/11 → 15 份 · 8/11 → 66 份
**发现**：`活动参与` **107 份全部为空**（从没人填过）→ 若不是刻意不用，可能前端没存进去，待查


## ✅ 发票「按学生」看板 + 已发送标记（2026-09-26）

**老板需求（原话确认版）**：发票页改成**以学生为主** —— 学生列表**只列名字**，右栏**只显示本月（按账期）的「已开具/未开具」+「已发送/未发送」**；**点进去**才看该学生**按账期分类的所有发票**，每行右侧也有发送状态。
**口径决定**：`这个月` = **按账期 `period`**（不是开票日、不是到期日）—— 因为 13 张「9月开票、8月账」的补开票，用开票月会变成"8月没开票"的假警报。
**已发送**：**只做手动标记**（不做自动）—— 老板说明：app 内 WhatsApp 发送时，费用项目常需要先改对再发，所以不能自动标已发送。

**数据结构**：`invoices` 新增 2 个字段 —— `sent`(bool) + `sent_at`(text)，迁移 `pb_migrations/1790402677_updated_invoices_sent.js`（已在生产库应用 ✓）。旧数据无值 → 全部显示"未发送"，点一次即记录。

**实现**
- 新组件 `app/components/finance/invoice-management/InvoiceByStudent.tsx`：学生表（学号/姓名/年级/本月开具/本月发送/查看）+ 账期选择器 + 搜索 + 「查看」弹窗（该生按账期分类的全部发票，可「标记已发送 / 取消标记」）
- `InvoiceManagement.tsx`：新增「列表 / 按学生」视图切换（默认列表，不影响原界面）

**实测（真 Chromium）**：112 位学生 ✓ 已开具 **108** / 未开具 **4**（与该账期实际一致 ✓）；点查看 → 弹出该生 2026-09 + 2026-08 两张票 ✓；**标记已发送 → 显示 2026-09-26 → 刷新后从数据库读回 `sent:true` ✓**；测试后已复原 ✓；零控制台错误 ✓
