...
const ctx=await b.newContext({viewport:{width:1500,height:1100},locale:'zh-CN'});
...
await pg.goto('http://127.0.0.1:3001/attendance',{waitUntil:'domcontentloaded'});
await pg.waitForTimeout(7000);
console.log('错误:', errs.length?errs:'无 ✅');
const t=await pg.evaluate(()=>document.body.innerText);
const i=t.indexOf('实时打卡');
console.log('--- 实时打卡面板 ---');
console.log(t.slice(i, i+520).replace(/\n{2,}/g,'\n'));
await b.close();
