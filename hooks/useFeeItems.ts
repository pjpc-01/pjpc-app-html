...
const fetchFeeItems = async (): Promise<FeeItem[]> => {
  ...
  try {
    const records = await pb.collection('fee_items').getFullList(200, {
      sort: 'category,name',
      fields: 'id,name,category,amount,description,status,frequency,applicableCenters,applicableLevels'
    })
    ...
    const feeItems: FeeItem[] = records.map(record => ({
      ...
      applicableCenters: record.applicableCenters || [],
    ...
    console.error('[useFeeItems] Error fetching fee items:', error)
    throw error
  }
}
