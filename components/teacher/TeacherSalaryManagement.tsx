                    ...
                    <TableRow key={structure.id} className={structureSelectedIds.has(structure.id) ? "bg-red-50/50" : ""}>
                      ...
                      <TableCell className="text-green-700 font-medium">
                        {(() => {
                          ...
                          // SOCSO First Schedule
                          const socso = gross <= 30 ? 0 : gross <= 50 ? 0.05 : gross <= 100 ? 0.10 : gross <= 200 ? 0.15 : gross <= 500 ? 0.25 : gross <= 1000 ? 0.35 : gross <= 2000 ? 0.45 : gross <= 3000 ? 0.55 : gross <= 4000 ? 0.65 : gross <= 5000 ? 0.75 : 0.85
                          const eis = Math.min(gross * (structure.eis_rate ?? 0.002), 2.45)
                          const tax = gross * (structure.tax_rate ?? 0)
                          return formatCurrency(gross - epf - socso - eis - tax)
                        })()}
                      </TableCell>
