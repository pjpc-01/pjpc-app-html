...
class _AddEditStudentScreenState extends State<AddEditStudentScreen> {
  ...
  Widget _buildRelationSelector(TextEditingController controller, TextEditingController nameController, TextEditingController phoneController) {
            ...
            return GestureDetector(
              onTap: () {
                setState(() {
                  ...
                  // 智能建议：如果是父母关系，自动填写家长信息
                  if (relation == '父亲' || relation == '母亲') {
                    _showParentSuggestion(relation, nameController, phoneController);
                  }
                });
              },
              child: Container(
  ...
  void _showParentSuggestion(String relation, TextEditingController nameController, TextEditingController phoneController) {
    // 检查家长信息是否已填写
    ...
    if (parentName.isNotEmpty || parentPhone.isNotEmpty) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('智能建议 - $relation'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('检测到您选择了"$relation"，是否要使用已填写的家长信息？'),
              const SizedBox(height: 12),
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('已自动填写$relation信息'),
                    backgroundColor: AppTheme.successColor,
                    duration: const Duration(seconds: 2),
                  ),
                );
              },
              child: const Text('使用家长信息'),
            ),
          ],
        ),
      );
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: AppSpacing.md),
                Expanded(
                  child: _buildRelationSelector(relationController, nameController, phoneController),
                ),
              ],
            ),
          ],
  ...
  Future<void> _scanNfcCard() async {
    try {
      ...
      if (nfcData != null && nfcData.isNotEmpty) {
        // 将扫描到的NFC ID填入表单
        setState(() {
          _nfcUrlController.text = nfcData!;
        });
