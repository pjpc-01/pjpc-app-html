...
export async function GET(request: NextRequest) {
  try {
    ...
    // 直接获取student_attendance集合的所有数据，不做分类
    ...
    console.log('🔍 学生考勤过滤条件:', filter)
    
    // 查询学生考勤记录
    const records = await pb.collection('student_attendance').getList(page, limit, {
      filter: filter,
      sort: '-created'
    })
    
    console.log('✅ 学生考勤查询成功，记录数:', records.items.length)
    console.log('🔍 学生ID列表:', records.items.map(r => r.student_id))
    ...
    if (existingRecords.items.length === 0) {
      ...
      if (existingRecord.check_out) {
        ...
        const attendanceData = {
          ...
          reason: reason || '',
          detail: detail || '',
          notes: `学生考勤系统 - ${method} (第${existingRecords.items.length + 1}次)`,
          teacher_id: teacher_id || 'system',
          teacher_name: teacher_name || '系统',
          device_info: device_info ? JSON.stringify(device_info) : '',
          method: method
        ...
        // 可以签退
        console.log('🔄 开始执行学生签退更新...')
        
        const updateData = {
          check_out: checkinTimestamp,
          notes: existingRecord.notes + ` | 学生考勤系统 - ${method}`,
