...
export async function PUT(request: NextRequest) {
  try {
    ...
    // 验证必需字段 - 支持两种字段名
    const status = replacementStatus || replacement_status
    if (!id || !status) {
      return NextResponse.json(
    ...
    // 添加新卡号（如果提供）
    const newCard = newCardNumber || new_card_number
    if (newCard) {
      updateData.new_card_number = newCard
    }
    
    // 添加备注（如果提供）
    const finalNotes = notes || replacementNotes
    if (finalNotes) {
      updateData.replacement_notes = finalNotes
    }
