    ...
    for (const teacher of teachers) {
      try {
        ...
        const schedules = await pb.collection('schedules').getList(1, 500, {
          filter: `teacher_id = "${teacher.id}" && date >= "${startDate}" && date < "${endExclusive}"`
        })
        ...
        if (schedules.items.length > 0 && scheduledDates.size > 0) {
          const attendanceList = await pb.collection('teacher_attendance').getList(1, 500, {
            filter: `teacher_id = "${teacher.id}" && check_in >= "${startDate}" && check_in < "${endExclusive}T23:59:59.999Z"`
          })
        ...
        const noStatutory = !!(structure as any).no_statutory
        const isMonthly = structure.salary_type !== 'hourly' && structure.salary_type !== 'commission'
        const epfDeduction = isMonthly && !noStatutory ? grossSalary * (structure.epf_rate || 0.11) : 0
        const socsoDeduction = isMonthly && !noStatutory ? calculateSOCSO(grossSalary) : 0
        const eisDeduction = isMonthly && !noStatutory ? calculateEIS(grossSalary) : 0
        const taxDeduction = isMonthly && !noStatutory ? calculatePCB(structure, grossSalary) : 0

        // 雇主缴纳（基于 grossSalary，仅月薪）
        const epfEmployer = isMonthly && !noStatutory ? grossSalary * (structure.epf_employer_rate || (grossSalary > 5000 ? 0.12 : 0.13)) : 0
        const socsoEmployer = isMonthly && !noStatutory ? calculateEmployerSOCSO(grossSalary) : 0
        const eisEmployer = isMonthly && !noStatutory ? calculateEIS(grossSalary) : 0
