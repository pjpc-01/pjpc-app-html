...
// Types
export interface ReceiptSettingsPreset {
  id: string
  name: string
  // School Info
  schoolName: string
...
const createDefaultPreset = (overrides?: Partial<ReceiptSettingsPreset>): ReceiptSettingsPreset => ({
...
// Generate a preview receipt HTML using current settings
export const generateReceiptPreviewHTML = (settings: ReceiptSettingsPreset): string => {
  ...
  const total = 1150.00
  const receiptNumber = "RCP-2026-06-12-PREVIEW"
  const receiptDate = "2026-06-12"
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .receipt-wrapper { width: 100%; background: #fff; overflow: hidden; }
    .header {
    ...
    .info-row { display: flex; justify-content: space-between; margin-bottom: 12px; gap: 24px; }
    .info-block { flex: 1; }
    .info-block h3 {
    ...
    <div class="body">
      ...
      <div class="info-row">
        <div class="info-block">
          ...
          <p class="highlight">${receiptNumber}</p>
        </div>
      ...
      ${settings.receiptNote ? `<div class="notes">📌 ${settings.receiptNote}</div>` : ''}
...
interface ReceiptSettingsManagerProps {
  onSettingsChange?: (settings: ReceiptSettingsPreset) => void
...
export default function ReceiptSettingsManager({ onSettingsChange, activePresetId }: ReceiptSettingsManagerProps) {
  ...
  useEffect(() => {
    ...
    const loadPresets = async () => {
      try {
        const res = await fetch('/api/pocketbase-proxy/api/collections/receipt_settings/records?perPage=50&sort=-created')
        ...
        console.error('Failed to load receipt settings from PB:', e)
      }

      // Fallback: try localStorage migration
      if (cancelled) return
      try {
        const saved = localStorage.getItem('receiptSettingsPresets')
        if (saved) {
          const parsed = JSON.parse(saved)
          if (Array.isArray(parsed) && parsed.length > 0) {
            for (const p of parsed) {
              await fetch('/api/pocketbase-proxy/api/collections/receipt_settings/records', {
                method: 'POST',
  ...
  const SAVE_FIELDS = [
    ...
    'schoolPhone', 'schoolEmail', 'primaryColor', 'secondaryColor', 'accentColor',
    'footerText', 'receiptNote', 'isDefault',
  ]

  const savePreset = async (preset: ReceiptSettingsPreset, isNew: boolean = false) => {
    const data: any = {}
    for (const f of SAVE_FIELDS) {
      if ((preset as any)[f] !== undefined) data[f] = (preset as any)[f]
    }
    const url = isNew
      ? '/api/pocketbase-proxy/api/collections/receipt_settings/records'
      : `/api/pocketbase-proxy/api/collections/receipt_settings/records/${preset.id}`
  ...
  const deletePreset = async (presetId: string) => {
    await fetch(`/api/pocketbase-proxy/api/collections/receipt_settings/records/${presetId}`, {
      method: 'DELETE'
    })
  }

  const updateSettings = async (updates: Partial<ReceiptSettingsPreset>) => {
