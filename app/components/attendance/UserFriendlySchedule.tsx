...
export default function UserFriendlySchedule() {
  ...
  // 获取可用的教师
  const getAvailableTeachers = (subject: string, grade: string) => {
  ...
  const autoSchedule = () => {
    ...
    weekDays.forEach(date => {
      ...
      if (!existingSchedule) {
        ...
        // 数学课
        const mathTeacher = getAvailableTeachers('数学', '四年级')[0]
        if (mathTeacher) {
          classes.push({
        ...
        // 英文课
        const englishTeacher = getAvailableTeachers('英文', '二年级')[0]
        if (englishTeacher) {
          classes.push({
