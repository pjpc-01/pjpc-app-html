...
export default function TeacherAttendanceManagement() {
  ...
  // 获取教师考勤记录
  const fetchTeacherAttendanceRecords = async () => {
    try {
      setLoading(true)
      setError(null)

      const searchParams = new URLSearchParams()
      searchParams.append('type', 'teacher')
      if (filters.date) searchParams.append('date', filters.date)
      if (filters.teacherName) searchParams.append('teacherName', filters.teacherName)
      ...
      const response = await fetch(`/api/teacher-attendance?${searchParams.toString()}`)
      ...
      if (data.success) {
        ...
        const present = data.records?.filter((r: TeacherAttendanceRecord) => r.status === 'present').length || 0
        const absent = data.records?.filter((r: TeacherAttendanceRecord) => r.status === 'absent').length || 0
        const late = data.records?.filter((r: TeacherAttendanceRecord) => r.status === 'late').length || 0
        const earlyLeave = data.records?.filter((r: TeacherAttendanceRecord) => r.status === 'early_leave').length || 0
        ...
        throw new Error(data.error || '获取教师考勤记录失败')
      }
    } catch (err) {
      console.error('获取教师考勤记录失败:', err)
      setError(err instanceof Error ? err.message : '获取教师考勤记录失败')
    } finally {
      setLoading(false)
    }
  }
  ...
  }, [currentPage, filters.date, filters.teacherName, filters.center, filters.status])
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex justify-between items-center mt-4">
            <Button
              variant="outline"
              onClick={fetchTeacherAttendanceRecords}
              disabled={loading}
            >
