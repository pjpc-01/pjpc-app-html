...
export default function StudentUrlsPage() {
  ...
  const visitStudentUrl = async (studentId: string, center: string) => {
    try {
      ...
      const student = cards.find(c => c.student_id === studentId)
      if (student?.student_url) {
        window.open(student.student_url, '_blank')
      }
    } catch (err) {
              ...
              <TableBody>
                {filteredStudents.map((student) => (
                  <TableRow key={student.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => copyToClipboard(student.student_url, student.student_id)}
                    ...
                    <TableCell>
                      <span className="text-sm">{student.usage_count || 0}</span>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => visitStudentUrl(student.student_id, student.center)}
